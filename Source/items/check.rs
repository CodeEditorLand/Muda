...
impl CheckMenuItem {
	...
	///   this character as the mnemonic for this check menu item. To display a
	///   `&` without assigning a mnemenonic, use `&&`.
	pub fn new<S: AsRef<str>>(text: S, enabled: bool, checked: bool, accelerator: Option<Accelerator>) -> Self {
		let item = crate::platform_impl::MenuChild::new_check(text.as_ref(), enabled, checked, accelerator, None);
